# Community Contribution For Exploratory Data Analysis: ggformat++
...
However, `ggformat` is not perfect -- although it works well for a single `ggplot2` sentence, it's power is limited because it cannot handle multiple sentences, long sentences and comments. Therefore, I propose `ggformat++` that can solve the aforementioned drawbacks of `ggformat`. 



#### 2. How it differs from `ggformat`

##### 2.1  Handle Multiple Sentences
...
##### 2.3 Wrapping Long Sentences

In the github of `ggformat`, it says that `ggformat` has trouble wrapping long sentences:

![1635206417090](C:\Users\zh-wa\AppData\Roaming\Typora\typora-user-images\1635206417090.png)
